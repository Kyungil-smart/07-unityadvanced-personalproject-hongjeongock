...
public class MainMenuController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 새 게임 버튼
        var btnNewGame = _mainRoot.Q<Button>("btn-newgame");
        if (btnNewGame != null)
        {
            btnNewGame.clicked += () =>
            {
                ...
                var slotController = saveSlotUI.GetComponent<SaveSlotMenuController>();
                slotController.SetNewGameMode(true);
            };
        }

        // 이어하기 버튼
        var btnContinue = _mainRoot.Q<Button>("btn-continue");
        if (btnContinue != null)
        {
            btnContinue.clicked += () =>
            {
        ...
        // 저장 폴더 버튼
        var btnSaveFile = _mainRoot.Q<Button>("btn-savefile");
        ...
        if (btnSaveFile != null)
        {
            btnSaveFile.clicked += () =>
            {
                ...
                var slotController = saveSlotUI.GetComponent<SaveSlotMenuController>();
                slotController.SetNewGameMode(false);
            };
        }

        // 설정 버튼
        var btnSettings = _mainRoot.Q<Button>("btn-settings");
        if (btnSettings != null)
        {
            btnSettings.clicked += () =>
            {
        ...
        // 크레딧 버튼
        var btnCredits = _mainRoot.Q<Button>("btn-credits");
        if (btnCredits != null)
        {
            btnCredits.clicked += () =>
            {
        ...
        // 종료 버튼
        var btnQuit = _mainRoot.Q<Button>("btn-quit");
        if (btnQuit != null)
        {
            btnQuit.clicked += () =>
            {
        ...
        if (btnClose != null)
            btnClose.clicked += OnCloseSettingsClicked;

        if (btnApply != null)
            btnApply.clicked += OnCloseSettingsClicked;

        if (_tabSound != null)
            _tabSound.clicked += () => SwitchTab("sound");

        if (_tabKeys != null)
            _tabKeys.clicked += () => SwitchTab("keys");
    ...
    /// 이어하기
    /// 나중에 저장 시스템 붙이면 여기서 불러오기 처리
    private void OnContinueClicked()
    {
        int lastSlot = PlayerPrefs.GetInt("LastSelectedSlot", 0);

        // 마지막 슬롯 정보가 없으면 중단
        if (lastSlot <= 0)
        {
            Debug.Log("마지막으로 플레이한 슬롯이 없습니다.");
            return;
        }

        // 해당 슬롯 저장 파일이 없으면 중단
        if (!SaveManager.HasSave(lastSlot))
        {
            Debug.Log("이어할 저장 데이터가 없습니다.");
            return;
        }

        Debug.Log($"이어하기: 슬롯 {lastSlot} 불러오기");
    ...
    private System.Collections.IEnumerator InitCredits()
    {
        ...
        if (closeTop != null)
            closeTop.clicked += OnCloseCreditsClicked;

        if (closeBottom != null)
            closeBottom.clicked += OnCloseCreditsClicked;
    }
